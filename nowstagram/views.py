...
@app.route('/profile/<int:user_id>/')  #个人页
@login_required    #验证登录权限
def profile(user_id):
...
def login():
    ...
    if user == None:
        return redirect_with_msg('/regloginpage/',u'用户名或密码不存在','reglogin')
...
def reg():
    ...
    salt = '.'.join(random.sample('0123456789qwertyuioASDFGHJKL',10))
    m = hashlib.md5()
